...
def submit_expense(conn: Connection, userid: int, amount: float, description: str, date: str):
    submit_query = ("INSERT INTO EXPENSE (userid, amount, description, date) "
                    "VALUES (%d, %f, '%s', '%s')") %(userid, amount, description, date)

    #also need to add to approvals
    approvals_query = ("INSERT INTO APPROVALS (expense_id, status) VALUES "
                       "(SELECT * FROM EXPENSE WHERE userid = %f, '%s'") %(userid, DEFAULT_STATUS)
    cursor = conn.cursor()
    try:
...
def edit_submission(conn: Connection, userid: int, expenseid: int, column: str, data: str):
    ...
    edit_query = (("UPDATE EXPENSE JOIN APPROVALS ON EXPENSE.id=APPROVALS.expense_id SET %s = %s WHERE "
                  "APPROVALS.status = '%s' AND EXPENSE.user_id = %d")
                  %(column, data, DEFAULT_STATUS, userid))
    cursor = conn.cursor()
